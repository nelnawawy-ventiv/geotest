define(["https://api.mapbox.com/mapbox-gl-js/v2.10.0/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v5.0.0/mapbox-gl-geocoder.min.js", "https://api.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.3.0/mapbox-gl-draw.js", "https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js",/*"https://code.jquery.com/ui/1.12.1/jquery-ui.min.js",*/ "text! https://cdn.jsdelivr.net/gh/nelnawawy-ventiv/geotest/Demobase7.css"], function (mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider1, slider2, Info) {
    ...
    BasicControl.prototype.initialize = function (oControlHost, fnDoneInitializing, oDataStore) {
        ...
        function myFunction10() {
            ...
            var visibility = map.getLayoutProperty('Residential Dwelling Density', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('Residential Dwelling Density', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('Residential Dwelling Density', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }

        }

        document.getElementById("v150_ValueComboBox").addEventListener("change", myFunctionUSWA);
        ...
        document.getElementById("v161_ValueComboBox").addEventListener("change", myFunctionUSFL);
        ...
        document.getElementById("v172_ValueComboBox").addEventListener("change", myFunctionAUSTRAF);
        ...
        document.getElementById("v183_ValueComboBox").addEventListener("change", myFunctionGCU);
        ...
        document.getElementById("v194_ValueComboBox").addEventListener("change", myFunctionMODIS);
        ...
        document.getElementById("v194_ValueComboBox").addEventListener("change", myFunctionVIC);

        function myFunctionVIC() {

            var visibility = map.getLayoutProperty('Emergency Incidents (VIC)', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('Emergency Incidents (VIC)', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('Emergency Incidents (VIC)', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }
    ...
    BasicControl.prototype.draw = function (oControlHost) {
        ...
        map.on('load', function () {
            map.addLayer({
                ...
                "source": {
                    "type": "raster",
                    "tiles": [
                        'http://services.ga.gov.au/gis/rest/services/NEXIS_Residential_Dwelling_Density/MapServer/export?bbox=-3850823.969990419%2C-5369043.910844071%2C3041740.5461386135%2C-694146.6273126616&bboxSR=&layers=show%3A3&layerDefs=&size=&imageSR=&historicMoment=&format=png&transparent=true&dpi=&time=&timeRelation=esriTimeRelationOverlaps&layerTimeOptions=&dynamicLayers=&gdbVersion=&mapScale=&rotation=&datumTransformations=&layerParameterValues=&mapRangeValues=&layerRangeValues=&clipping=&spatialFilter=&size=256,256&f=image'
        ...
        map.on('load', function () {
            map.addLayer({
                ...
                'source': {
                    'type': 'geojson',
                    'data': 'https://mrgis.mainroads.wa.gov.au/arcgis/rest/services/OpenData/RoadSafety_DataPortal/MapServer/2/query?where=1%3D1&text=&objectIds=&time=&timeRelation=esriTimeRelationOverlaps&geometry=&geometryType=esriGeometryPolygon&inSR=&spatialRel=esriSpatialRelIntersects&distance=&units=esriSRUnit_Foot&relationParam=&outFields=&returnGeometry=true&returnTrueCurves=false&maxAllowableOffset=&geometryPrecision=&outSR=&havingClause=&returnIdsOnly=false&returnCountOnly=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&returnZ=false&returnM=false&gdbVersion=&historicMoment=&returnDistinctValues=false&resultOffset=&resultRecordCount=&returnExtentOnly=false&sqlFormat=none&datumTransformation=%5B3035%2C+3857%5D&parameterValues=&rangeValues=&quantizationParameters=&featureEncoding=esriDefault&f=geojson'
        ...
        map.on('click', function (e) {
            ...
            var popup = new mapboxgl.Popup({ offset: [0, -15] })
                .setLngLat(feature.geometry.coordinates)
                .setHTML('<h3>' + feature.properties.INTERSECTION_DESC + '</h3><p>' + '<br>' + 'Accident Type: ' + feature.properties.ACCIDENT_TYPE + '<br>' + 'Crash Date: ' + feature.properties.CRASH_DATE + '<br>' + 'Severity: ' + feature.properties.SEVERITY + '<br>' + 'Cause: ' + feature.properties.EVENT_NATURE + '<br>' + '</p>')
                .setLngLat(feature.geometry.coordinates)
                .addTo(map);
        ...
        */
        ...
        map.on('load', function () {
            ...
            map.addSource('emergencyVIC', {
                type: 'geojson',
                data: 'https://data.emergency.vic.gov.au/Show?pageId=getIncidentJSON'
            });
            map.addLayer({
        ...
        map.on('click', function (e) {
            ...
            var features = map.queryRenderedFeatures(e.point, {
                layers: ['Emergency Incidents'] // replace this with the name of the layer
            });
        ...
        // Change the cursor to a pointer when the mouse is over the places layer.
        map.on('mouseenter', 'Emergency Incidents', function () {
            map.getCanvas().style.cursor = 'pointer';
        });

        // Change it back to a pointer when it leaves.
        map.on('mouseleave', 'Emergency Incidents', function () {
            map.getCanvas().style.cursor = '';
        });
    ...
    BasicControl.prototype.setData = function (oControlHost, oDataStore) {
        ...
        for (var iRow = 0; iRow < iRowCount; iRow++) {
            ...
            avghouse = oDataStore.getCellValue(iRow, 13)
            sealevel2030 = oDataStore.getCellValue(iRow, 14)
            globalforecast = oDataStore.getCellValue(iRow, 15)
            crash5yr = oDataStore.getCellValue(iRow, 16)
            austraffic = oDataStore.getCellValue(iRow, 17)
            civilunrest = oDataStore.getCellValue(iRow, 18)
            MODIS = oDataStore.getCellValue(iRow, 19)
