define(["https://api.mapbox.com/mapbox-gl-js/v2.10.0/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v5.0.0/mapbox-gl-geocoder.min.js", "https://api.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.3.0/mapbox-gl-draw.js", "https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js",/*"https://code.jquery.com/ui/1.12.1/jquery-ui.min.js",*/ "text! https://cdn.jsdelivr.net/gh/nelnawawy-ventiv/geotest/Demobase7h.css"/*, "https://cdn.jsdelivr.net/npm/chart.js@3.9.1/dist/chart.min.js"*/], function (mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider1, slider2, Info) {
    ...
    BasicControl.prototype.initialize = function (oControlHost, fnDoneInitializing, oDataStore) {
        ...
        document.getElementById("v171_ValueComboBox").addEventListener("change", myFunctionAUSTRAF);
        ...
        document.getElementById("v182_ValueComboBox").addEventListener("change", myFunctionGCU);
        ...
        document.getElementById("v193_ValueComboBox").addEventListener("change", myFunctionCRiMENSW);

        function myFunctionCRiMENSW() {

            var visibility = map.getLayoutProperty('Crime Hotspots 2021 (NSW)', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('Crime Hotspots 2021 (NSW)', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('Crime Hotspots 2021 (NSW)', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }

        }

        document.getElementById("v204_ValueComboBox").addEventListener("change", myFunctionMODIS);
        ...
        document.getElementById("v215_ValueComboBox").addEventListener("change", myFunctionVIC);
        ...
        document.getElementById("v226_ValueComboBox").addEventListener("change", myFunctionWSA);
        ...
        document.getElementById("v237_ValueComboBox").addEventListener("change", myFunctionVOL);
        ...
        document.getElementById("v248_ValueComboBox").addEventListener("change", myFunctionRADAR);
        ...
        document.getElementById("v259_ValueComboBox").addEventListener("change", myFunctionTSUNAMI);
    ...
    BasicControl.prototype.draw = function (oControlHost) {
        ...
        map.on('load', function () {
            ...
            // Change it back to a pointer when it leaves.
            map.on('render', 'Australia Land Use', function () {
                jQuery('body .mapboxgl-control-container').append('<div id=' + 'main-legend' + ' ' + 'class=' + 'legend' + '>' + '<h4>' + 'Legend' + '</h4>' + '<div>' + '<span style=' + '"background-color: #ca7bf5"' + '; padding: 10px 10px 5px 10px; left: auto; margin: auto;">' + '</span>' + 'Conservation' + '</div>' + '<div>' + '<span style=' + '"background-color: #ffffbe"' + '>' + '</span>' + 'Natural Environments' + '</div>' + '<div>' + '<span style=' + '"background-color: #ffa900"' + '>' + '</span>' + 'Dryland Agriculture' + '</div>' + '<div>' + '<span style=' + '"background-color: #744d00"' + '>' + '</span>' + 'Irrigated Agriculture' + '</div>' + '<div>' + '<span style=' + '"background-color: #ff0000"' + '>' + '</span>' + 'Intensive Uses' + '</div>' + '<div>' + '<span style=' + '"background-color: #0000ff"' + '>' + '</span>' + 'Water' + '</div>');
            });
        ...
        map.on('load', function () {
            ...
            map.addSource('NSWCrime2021-cdpdlj', {
                type: 'vector',
                url: 'mapbox://nevnd.0p61cos1' //fxhawk.b7tr8njd'
            });
            map.addLayer({
                "id": "Crime Hotspots 2021 (NSW)",
                "type": "fill",
                "source": "NSWCrime2021-cdpdlj",
                "source-layer": "NSWCrime2021-cdpdlj",
                "layout": {
                    "visibility": crimensw
                },
                "paint": {
        ...
        map.on('click', function (e) {
            ...
            var features = map.queryRenderedFeatures(e.point, {
                layers: ["Crime Hotspots 2021(NSW)"] // replace this with the name of the layer
            });
            ...
            var popup = new mapboxgl.Popup({ offset: [0, -15] })
                .setLngLat(feature.geometry.coordinates)
                .setHTML('<h3>' + feature.properties.Crime_Type + '</h3><p>' + 'Crime Occurrence: ' + feature.properties.Contour + '<br>' + 'Crime Density Scale: ' + feature.properties.Density + '</p>')
                //.setLngLat(feature.geometry.coordinates)
                .addTo(map);
        ...
        // Change the cursor to a pointer when the mouse is over the places layer.
        map.on('mouseenter', "Crime Hotspots 2021 (NSW)", function () {
            map.getCanvas().style.cursor = 'pointer';
        });

        // Change it back to a pointer when it leaves.
        map.on('mouseleave', "Crime Hotspots 2021 (NSW)", function () {
            map.getCanvas().style.cursor = '';
        });
    ...
    BasicControl.prototype.setData = function (oControlHost, oDataStore) {
        ...
        for (var iRow = 0; iRow < iRowCount; iRow++) {
            ...
            feature['properties'] = { 'name': oDataStore.getCellValue(iRow, 0), 'total_paid3': oDataStore.getCellValue(iRow, 28), 'Net_Incurred_Total': oDataStore.getCellValue(iRow, 29) }
            ...
            Business_Unit = oDataStore.getCellValue(iRow, 20)
            Coverage = oDataStore.getCellValue(iRow, 21)
            auswetsurface = oDataStore.getCellValue(iRow, 22)
            volcanerup = oDataStore.getCellValue(iRow, 23)
            rainviewer = oDataStore.getCellValue(iRow, 24)
            activehur = oDataStore.getCellValue(iRow, 25)
            crimensw = oDataStore.getCellValue(iRow, 26)
            tsunami = oDataStore.getCellValue(iRow, 27)
