define(["https://api.mapbox.com/mapbox-gl-js/v2.10.0/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v5.0.0/mapbox-gl-geocoder.min.js", "https://api.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.3.0/mapbox-gl-draw.js", "https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js",/*"https://code.jquery.com/ui/1.12.1/jquery-ui.min.js",*/ "text! https://cdn.jsdelivr.net/gh/nelnawawy-ventiv/geotest/Demobase7h.css"], function (mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider1, slider2, Info) {
    ...
    BasicControl.prototype.initialize = function (oControlHost, fnDoneInitializing, oDataStore) {
        ...
        document.getElementById("v149_ValueComboBox").addEventListener("change", myFunctionGLWA);

        function myFunctionGLWA() {
        ...
        document.getElementById("v171_ValueComboBox").addEventListener("change", myFunctionEUFL);
        ...
        document.getElementById("v193_ValueComboBox").addEventListener("change", myFunctionUSSNOW);
    ...
    BasicControl.prototype.draw = function (oControlHost) {
        ...
        map.on('click', 'Hazardous Weather - Position', function (e) {
            ...
            new mapboxgl.Popup()
                .setLngLat(e.lngLat)
                .setHTML('<h3>' + 'Storm Name: ' + e.features[0].properties.STORMNAME + '</h3><p>' + 'Date: ' + e.features[0].properties.MONTH + e.features[0].properties.DAY + e.features[0].properties.YEAR + '<br>' + 'Intensity: ' + e.features[0].properties.INTENSITY + '</p>')
                .addTo(map);
        });
        ...
        map.on('click', 'Hazardous Weather - Tracks', function (e) {
            ...
            new mapboxgl.Popup()
                .setLngLat(e.lngLat)
                .setHTML('<h3>' + 'Storm Name: ' + e.features[0].properties.STORMNAME + '</h3><p>' + 'Date: ' + e.features[0].properties.STARTDTG + '<br>' + 'Storm Type: ' + e.features[0].properties.STORMTYPE + '</p>')
                .addTo(map);
        });
            ...
            fetch("https://api.rainviewer.com/public/weather-maps.json")
                ...
                .then(apiData => {
                    apiData.radar.past.forEach(frame => {
                        map.addLayer({
                            id: 'Radar', //`rainviewer_${frame.path}`,
                            type: "raster",
                            source: {
                                type: "raster",
                                tiles: [
    ...
    BasicControl.prototype.setData = function (oControlHost, oDataStore) {
        ...
        for (var iRow = 0; iRow < iRowCount; iRow++) {
            ...
            feature['properties'] = { 'name': oDataStore.getCellValue(iRow, 0), 'total_paid3': oDataStore.getCellValue(iRow, 21) }
            ...
            rainviewer = oDataStore.getCellValue(iRow, 11)
            claims = oDataStore.getCellValue(iRow, 12)
            claim_info = oDataStore.getCellValue(iRow, 13)
            sealevel2030 = oDataStore.getCellValue(iRow, 14)
            globalforecast = oDataStore.getCellValue(iRow, 15)
            heatstress_eu = oDataStore.getCellValue(iRow, 16)
            flooding_eu = oDataStore.getCellValue(iRow, 17)
            civilunrest = oDataStore.getCellValue(iRow, 18)
            temp_eu = oDataStore.getCellValue(iRow, 19)
            MODIS = oDataStore.getCellValue(iRow, 20)
