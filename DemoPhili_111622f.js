define(["https://api.mapbox.com/mapbox-gl-js/v2.10.0/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v5.0.0/mapbox-gl-geocoder.min.js", "https://api.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.3.0/mapbox-gl-draw.js", "https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js",/*"https://code.jquery.com/ui/1.12.1/jquery-ui.min.js",*/ "text! https://cdn.jsdelivr.net/gh/nelnawawy-ventiv/geotest/Demobase7h.css", "https://d3js.org/d3.v3.min.js"/* "https://cdn.jsdelivr.net/npm/chart.js@3.9.1/dist/chart.min.js"*/], function (mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider1, slider2, Info, d3) {
    ...
    BasicControl.prototype.initialize = function (oControlHost, fnDoneInitializing, oDataStore) {
        ...
        document.getElementById("v203_ValueComboBox").addEventListener("change", myFunctionIL100yr);

        function myFunctionIL100yr() {

            var visibility = map.getLayoutProperty('Ilocos Norte 100YR Floodplain', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('Ilocos Norte 100YR Floodplain', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('Ilocos Norte 100YR Floodplain', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }

        }

        document.getElementById("v214_ValueComboBox").addEventListener("change", myFunctionILLD);

        function myFunctionILLD() {

            var visibility = map.getLayoutProperty('Ilocos Norte Landslide Hazard', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('Ilocos Norte Landslide Hazard', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('Ilocos Norte Landslide Hazard', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }

        }

        document.getElementById("v225_ValueComboBox").addEventListener("change", myFunctionRADAR);

        function myFunctionRADAR() {

            var visibility = map.getLayoutProperty(`rainviewer_${frame.path}`, 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty(`rainviewer_${frame.path}`, 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty(`rainviewer_${frame.path}`, 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }
    ...
    BasicControl.prototype.draw = function (oControlHost) {
        ...
        map.on('load', function () {
            ...
            map.addSource('IlocosNorte-100yr-6prh72', {
                type: 'vector',
                url: 'mapbox://nevnd.1gorwf3k' //nevnd.ckr0tdn0u4rov20to1aq9z17x-5p275' //fxhawk.cjhhzt5rk00hw2xro80s4lgdy-8ybmv'
            });
            map.addLayer({
                'id': 'Ilocos Norte 100YR Floodplain',
                "type": "fill",
                "source": "IlocosNorte-100yr-6prh72",
                "source-layer": "IlocosNorte-100yr-6prh72",
                "layout": {
                    "visibility": il100yr
                },
                "paint": {
        ...
        map.on('click', function (e) {
            ...
            var popup = new mapboxgl.Popup({ offset: [0, -15] })
                .setLngLat(feature.geometry.coordinates)
                .setHTML('<h3>' + feature.properties.Name + '</h3>' +'<p>' + 'Capacity: ' + feature.properties.Capacity + '<br>' + 'Type: ' + feature.properties.Type + '</p>')
                .setLngLat(feature.geometry.coordinates)
                .addTo(map);
        });
        // Change the cursor to a pointer when the mouse is over the places layer.
        ...
        map.on('click', function (e) {
            ...
            var popup = new mapboxgl.Popup({ offset: [0, -15] })
                .setLngLat(feature.geometry.coordinates)
                .setHTML('<h3>' + feature.properties.Name + '</h3>' + 'Capacity: ' + feature.properties.Capacity + '<br>' + 'Type: ' + feature.properties.Type + '</p>')
                .setLngLat(feature.geometry.coordinates)
                .addTo(map);
        });
        // Change the cursor to a pointer when the mouse is over the places layer.
    ...
    BasicControl.prototype.setData = function (oControlHost, oDataStore) {
        ...
        for (var iRow = 0; iRow < iRowCount; iRow++) {
            ...
            il100yr = oDataStore.getCellValue(iRow, 22)
            ilnorteldslide = oDataStore.getCellValue(iRow, 23)
