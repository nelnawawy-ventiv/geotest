define(["https://api.mapbox.com/mapbox-gl-js/v2.9.2/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v5.0.0/mapbox-gl-geocoder.min.js", "https://api.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.3.0/mapbox-gl-draw.js", "https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js",/*"https://code.jquery.com/ui/1.12.1/jquery-ui.min.js",*/ "text! https://cdn.jsdelivr.net/gh/nelnawawy-ventiv/geotest/Demobase7.css"], function (mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider1, slider2, Info) {
    ...
    BasicControl.prototype.initialize = function (oControlHost, fnDoneInitializing, oDataStore) {
        ...
        document.getElementById("v228_ValueComboBox").addEventListener("change", myFunctionUSoutlook);

        function myFunctionUSoutlook() {

            var visibility = map.getLayoutProperty('US Temp Outlook - 6-10 Day', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('US Temp Outlook - 6-10 Day', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('US Temp Outlook - 6-10 Day', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }
    ...
    BasicControl.prototype.draw = function (oControlHost) {
        ...
        map.on('load', function () {
            map.addLayer({
                "id": "US Temp Outlook - 6-10 Day",
                "type": "raster",
                "minzoom": 0,
                "maxzoom": 22,
                "source": {
                    "type": "raster",
                    "tiles": ['https://idpgis.ncep.noaa.gov/arcgis/rest/services/NWS_Climate_Outlooks/cpc_6_10_day_outlk/MapServer/export?dpi=96&bbox=-293.7692491671929,-328.4203216326983,80.39101937471492,439.0178934058135&layers=show:0&transparent=true&format=png32&bboxSR=EPSG:3857&imageSR=EPSG:3857&size=256,256&f=image'],
                    "tileSize": 256,
                    "bounds": [-293.7692491671929, -328.4203216326983, 80.39101937471492, 439.0178934058135]
        ...
        */
        ...
        map.on('load', function () {
            map.addLayer({
                "id": "US Temperatures - Highs",
                "type": "raster",
                "minzoom": 0,
                "maxzoom": 22,
                "source": {
                    "type": "raster",
                    "tiles": ['https://idpgis.ncep.noaa.gov/arcgis/rest/services/NWS_Forecasts_Guidance_Warnings/NDFD_temp/MapServer/export?bbox=-129.72946923559348,4.0654558126074285,-83.74638529792335,66.47106972801691&dpi=96&transparent=true&bboxSR=EPSG:3857&layers=show:124&format=png32&f=image'],//'https://idpgis.ncep.noaa.gov/arcgis/rest/services/NWS_Forecasts_Guidance_Warnings/NDFD_temp/MapServer/export?bbox=-129.72946923559348,4.0654558126074285,-83.74638529792335,66.47106972801691&dpi=96&transparent=true&bboxSR=EPSG:3857&layers=show:124&format=png32&size=256,256&f=image'],//export?bbox=-129.72946923559348,4.0654558126074285,-83.74638529792335,66.47106972801691'],
                    "tileSize": 256

                }, 'layout': { 'visibility': maxustemp },
            });
    ...
    BasicControl.prototype.setData = function (oControlHost, oDataStore) {
        ...
        for (var iRow = 0; iRow < iRowCount; iRow++) {
            ...
            ustempout = oDataStore.getCellValue(iRow, 22)
