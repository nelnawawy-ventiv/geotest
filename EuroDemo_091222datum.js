define(["https://api.mapbox.com/mapbox-gl-js/v2.10.0/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v5.0.0/mapbox-gl-geocoder.min.js", "https://api.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.3.0/mapbox-gl-draw.js", "https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js",/*"https://code.jquery.com/ui/1.12.1/jquery-ui.min.js",*/ "text! https://cdn.jsdelivr.net/gh/nelnawawy-ventiv/geotest/Demobase7.css"], function (mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider1, slider2, Info) {
    ...
    BasicControl.prototype.initialize = function (oControlHost, fnDoneInitializing, oDataStore) {
        ...
        function myFunction6() {
            ...
            var visibility = map.getLayoutProperty('Water Scarcity (2071-2100)', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('Water Scarcity (2071-2100)', 'visibility', 'none');
                //document.getElementById("v90_MultiSelectList").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('Water Scarcity (2071-2100)', 'visibility', 'visible');
        ...
        document.getElementById("v138_ValueComboBox").addEventListener("change", sealevel);
        ...
        function myFunction10() {
            ...
            var visibility = map.getLayoutProperty('Projected Fire (2071-2100)', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('Projected Fire (2071-2100)', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('Projected Fire (2071-2100)', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }

        }

        document.getElementById("v162_ValueComboBox").addEventListener("change", myFunctionGLFC);

        function myFunctionGLFC() {

            var visibility = map.getLayoutProperty('Global Forecast', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('Global Forecast', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('Global Forecast', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }
        ...
        function myFunctionUSFL() {
            ...
            var visibility = map.getLayoutProperty('Heat Stress (2031-2060)', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('Heat Stress (2031-2060)', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('Heat Stress (2031-2060)', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }
        ...
        document.getElementById("v206_ValueComboBox").addEventListener("change", maxtempchange);

        function maxtempchange() {

            var visibility = map.getLayoutProperty('Temperature Change (2051-2100)', 'visibility');

            if (visibility === 'visible') {
                map.setLayoutProperty('Temperature Change (2051-2100)', 'visibility', 'none');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#9BA3A5';

            } else {

                map.setLayoutProperty('Temperature Change (2051-2100)', 'visibility', 'visible');
                //document.getElementById("v138_ValueComboBox").style.backgroundColor = '#EFE013';
            }
        ...
        map = new mapboxgl.Map({
            ...
            center: [13.0464, 47.7591],//[-.165748, 51.507], //Center Map
    ...
    BasicControl.prototype.draw = function (oControlHost) {
        ...
        map.on('load', function () {
            map.addLayer({
                "id": "Water Scarcity (2071-2100)",
                "type": "raster",
                "minzoom": 0,
                "maxzoom": 22,
                "source": {
                    "type": "raster",
                    "tiles": ['https://climate.discomap.eea.europa.eu/arcgis/rest/services/Drought/droughts_water_scarcity/MapServer/export?dpi=96&transparent=true&format=png32&layers=show%3A2&bbox={bbox-epsg-3857}&bboxSR=EPSG:3857&imageSR=EPSG:3857&size=256,256&f=image'],
                    "tileSize": 256

                }, 'layout': { 'visibility': waterscarcity },
                'paint': {
                    'raster-opacity': 0.7
                }
            });
        ...
        map.on('click', 'Projected Fire (2071-2100)', function (e) {
            //var Day = new Date(e.features[0].properties.FireDiscoveryDateTime);
            //var Day1 = Day.toUTCString();

            new mapboxgl.Popup()
                .setLngLat(e.lngLat)
                .setHTML('<h3>' + 'ID: ' + e.features[0].properties.Id + '</h3><p>' + 'Projected Fire Danger 2071-2100 Annual Average' + '<br>' + 'Seasonal Severity Rating: ' + e.features[0].properties.Y_71_00 + '</p>')
                .addTo(map);
        });

        // Change the cursor to a pointer when the mouse is over the states layer.
        map.on('mouseenter', 'Projected Fire (2071-2100)', function () {
            map.getCanvas().style.cursor = 'pointer';
        });

        // Change it back to a pointer when it leaves.
        map.on('mouseleave', 'Projected Fire (2071-2100)', function () {
            map.getCanvas().style.cursor = '';
        });
        ...
        map.on('load', function (e) {
            $.ajax({
                url: 'https://weather.ls.hereapi.com/weather/1.0/report.json?product=observation&apiKey=CKz7uMRfho2KvgDaJqJYqAUCGL_8R8dpugs8wHEELLQ' + 'latitude=' + coordy + '&longitude=' + coordx,
                type: 'GET',
                dataType: 'jsonp',
                jsonp: 'jsonpcallback',
                data: {
                    ...
                    oneobservation: 'true',
                    apiKey: 'CKz7uMRfho2KvgDaJqJYqAUCGL_8R8dpugs8wHEELLQ'
                },
                //proccessData: false,
        ...
        map.on('load', function () {
            map.addLayer({
                ...
                'source': {
                    'type': 'raster',
                    'tiles': [
                        'https://climate.discomap.eea.europa.eu/arcgis/rest/services/Heatstress/Heatstress_in_sheep/MapServer/export?dpi=96&transparent=true&format=png32&layers=show%3A0&bbox={bbox-epsg-3857}&bboxSR=EPSG:3857&imageSR=EPSG:3857&size=256,256&f=image'],
                ...
                'layout': { 'visibility': heatstress_eu },
                'paint': {
                    'raster-opacity': 0.7,
                    }
            });
        ...
        map.on('load', function () {
            map.addLayer({
                ...
                "source": {
                    "type": "raster",
                    "tiles": ['https://climate.discomap.eea.europa.eu/arcgis/rest/services/Urban_Vulnerability/Temperatures_Change_max/MapServer/export?dpi=96&transparent=true&format=png32&layers=show%3A0&bbox={bbox-epsg-3857}&bboxSR=EPSG:3857&imageSR=EPSG:3857&size=256,256&f=image'], //3035
                    "tileSize": 256
        ...
        */
        ...
        map.on('load', function () {
            map.addLayer({
                "id": "Projected Fire (2071-2100)",
                "type": "raster",
                "minzoom": 0,
                "maxzoom": 22,
                "source": {
                    "type": "raster",
                    "tiles": ['https://climate.discomap.eea.europa.eu/arcgis/rest/services/Forest_Fires/FF_projections/MapServer/export?bbox=2848457.861952167%2C2887506.1341055846%2C6527660.323397649%2C5524655.999666982&bboxSR=&layers=&layerDefs=&size=&imageSR=&historicMoment=&format=png&transparent=false&dpi=&time=&timeRelation=esriTimeRelationOverlaps&layerTimeOptions=&dynamicLayers=&gdbVersion=&mapScale=&rotation=&datumTransformations=%5B3035%2C+3857%5D&layerParameterValues=&mapRangeValues=&layerRangeValues=&clipping=&spatialFilter=&f=image'], //'https://climate.discomap.eea.europa.eu/arcgis/rest/services/Forest_Fires/FF_projections/MapServer/export?dpi=96&transparent=true&format=png32&layers=show%3A0&bbox={bbox-epsg-3857}&bboxSR=EPSG:3857&imageSR=EPSG:3857&size=256,256&f=image'], //'https://services3.arcgis.com/T4QMspbfLg3qTGWY/arcgis/rest/services/Public_Wildfire_Perimeters_View/FeatureServer/0/query?where=1%3D1&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&resultType=none&distance=0.0&units=esriSRUnit_Meter&returnGeodetic=false&outFields=*&returnGeometry=true&returnCentroid=false&featureEncoding=esriDefault&multipatchOption=xyFootprint&maxAllowableOffset=&geometryPrecision=&outSR=&datumTransformations=%5B3035%2C+3857%5D&applyVCSProjection=false&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnExtentOnly=false&returnQueryGeometry=false&returnDistinctValues=false&cacheHint=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&having=&resultOffset=&resultRecordCount=&returnZ=false&returnM=false&returnExceededLimitFeatures=true&quantizationParameters=&sqlFormat=none&f=pgeojson&token='
                    "tileSize": 256
    ...
    BasicControl.prototype.setData = function (oControlHost, oDataStore) {
        ...
        for (var iRow = 0; iRow < iRowCount; iRow++) {
            ...
            waterscarcity = oDataStore.getCellValue(iRow, 9)
            ...
            sealevel2030 = oDataStore.getCellValue(iRow, 15)
            globalforecast = oDataStore.getCellValue(iRow, 16)
            heatstress_eu = oDataStore.getCellValue(iRow, 17)
            flooding_eu = oDataStore.getCellValue(iRow, 18)
            civilunrest = oDataStore.getCellValue(iRow, 19)
            temp_eu = oDataStore.getCellValue(iRow, 20)
