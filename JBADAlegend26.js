define(["https://api.tiles.mapbox.com/mapbox-gl-js/v2.3.1/mapbox-gl.js", "jquery", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v4.7.0/mapbox-gl-geocoder.min.js", "https://api.tiles.mapbox.com/mapbox.js/plugins/turf/v3.0.11/turf.min.js", "https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.2.2/mapbox-gl-draw.js", "https://ajax.aspnetcdn.com/ajax/jQuery/jquery-3.6.0.min.js", "https://code.jquery.com/ui/1.12.1/jquery-ui.min.js", "text! https://cdn.jsdelivr.net/gh/Xdev9/Geospatial/Mapbox_12.css"], function (mapboxgl, jQuery, MapboxGeocoder, turf, MapboxDraw, slider1, slider2, Info) {
    "use strict";
    var map = '',
        bounds = '',
        geojsonFeature = {};
    ...
    BasicControl.prototype.initialize = function (oControlHost, fnDoneInitializing, oDataStore) {
        ...
        jQuery("head link[rel='stylesheet']").last().after("<link href='https://api.tiles.mapbox.com/mapbox-gl-js/v2.3.0/mapbox-gl.css' rel='stylesheet' />");
        ...
        jQuery("body").prepend("<link rel='stylesheet' href='https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-draw/v1.2.2/mapbox-gl-draw.css' type='text/css'/>");
        jQuery("tbody").after("<link rel='stylesheet' href='https://cdn.jsdelivr.net/gh/nelnawawy-ventiv/geotest/MapboxL_4.css' rel='stylesheet' />");
        jQuery("head link[rel='stylesheet']").last().after("<link href='https://cdn.jsdelivr.net/gh/nelnawawy-ventiv/geotest/MapboxL_3.css' type='text/css' />");
    ...
    BasicControl.prototype.draw = function (oControlHost) {
        ...
        jQuery('body .mapboxgl-control-container').append('<div id=' + 'console' + '>' + '<div>' + '<div class=' + 'session' + ' ' + 'id=' + 'sliderbar' + ' ' + '>' + ' ' + '<p>' + '<font color=' + /*"yellow"*/"efbf13" + '>' + 'Historical - Year: ' + '<label id=' + 'active-Year' + '>' + '2013' + '</label>' + '</font>' +
            '<input id=' + 'slider' + ' ' + 'class=' + 'row' + ' ' + 'type=' + 'range' + ' ' + 'min=' + '2013' + ' ' + 'max=' + '2019' + ' ' + 'step=' + '1' + ' ' + 'value=' + '2013' + '/>' + '</p>' + '</div>');
        jQuery('body .mapboxgl-control-container').append('<div class=' + 'calculation-box' + '>' + '<p>' + 'Create boundaries with the draw tool to select markers' + '</p>' + '<div id=' + 'calculated-area' + '>' + '</div>'); + '</div>'
        jQuery('body .mapboxgl-control-container').append('<div id=' + 'main-legend' + ' ' + 'class=' + 'legend' + '>' + '<h4>' + 'Legend' + '</h4>' + '<div>' + '<span style=' + '"background-image:' + " url('https://cdn.rawgit.com/mapbox/mapbox-gl-styles/master/sprites/bright-v9/_svg/town-hall-11.svg')" + '; padding: 10px 10px 5px 10px; left: auto; margin: auto;">' + '</span>' + 'Properties' + '</div>' + '<div>' + '<span style=' + '"background-image:' + " url('https://cdn.rawgit.com/mapbox/mapbox-gl-styles/master/sprites/bright-v9/_svg/castle-11.svg')" + '; padding: 10px 10px 5px 10px; left: auto; margin: auto;">' + '</span>' + 'Defended Areas' + '</div>' + '<div>' + '<span style=' + '"background-color: #efbf13"' + '>' + '</span>' + 'Claims' + '</div>' + '<div>' + '<span style=' + '"background-color: #f80a0a"' + '>' + '</span>' + 'Wildfire' + '</div>' + '<div>' + 'Storm Surge (meters)' + '<div>' + '<span style=' + '"background-color: #2d97fb"' + '>' + '</span>' + '0 to 0.09m' + '</div>' + '<div>' + '<span style=' + '"background-color: #6520f8"' + '>' + '</span>' + '0.1m' + '</div>' + '<div>' + '<span style=' + '"background-color: #8404a4"' + '>' + '</span>' + '0.7m' + '</div>' + '</div>');
        ...
        /*map.addLayer({
            ...
            "source": {
                "type": "json",
                "data": //'https://coast.noaa.gov/arcgis/rest/services/FloodExposureMapper/CFEM_FEMAFloodZones/MapServer?f=pjson' //'https://coast.noaa.gov/arcgis/rest/services/FloodExposureMapper/CFEM_FEMAFloodZones/MapServer/0'
        ...
        map.on('load', function () {
            ...
            map.addSource('stu1500-94rgde', { //'CASTSU1500B-9alk2o', {
                type: 'vector',
                url: 'mapbox://nevnd.9kctmzz3', //'mapbox://nevnd.dm23d4er',
                //tileSize: 256
            });
            map.addLayer({
                "id": "Flooding",
                "type": "vector",
                "source": "stu1500-94rgde", //"CASTSU1500B-9alk2o",
                //"source-layer": "CASTSU1500B-9alk2o",
                "minzoom": 12,
                "maxzoom": 22,
                "layout": {
                ...
                "paint": {
                    ...
                    "raster-hue-rotate": [150, 180, 210, 240]
                }
        ...
        //map.on('load', function () {
            ...
            var layers = ['Properties', 'Flood Defended Areas', 'Claims', 'Wildfire'];
            var symbols = ["town-hall-11", "castle-11", "#efbf13", "#f80a0a" ];

            for (i = 0; i < layers.length; i++) {
                var layer = layers[i];
                var symbols = symbols[i];
                var item = document.createElement('div');
                var key = document.createElement('span');
                key.className = 'legend-key';
                key.style.backgroundImage = symbols;
            ...
            map.on('mousemove', function (e) {
                var laybels = map.queryRenderedFeatures(e.point, {
                    layers: ['Properties2', 'Flood Defended Areas', 'points', 'Wildfire']
                });

                if (laybels.length > 0) {
                    document.getElementById('pd').innerHTML =
                        '<h3><strong>' +
                        laybels[0].properties.name +
                        '</strong></h3><p><strong><em>'
                } else {
                    document.getElementById('pd').innerHTML =
                        '<p>Hover over a layer!</p>';
                }
            });*/
            //var legend = document.getElementById('main-legend');
    ...
    return BasicControl;
}); // JavaScript source code// JavaScript source code// JavaScript source code
